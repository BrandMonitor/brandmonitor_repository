...
<html lang="en">
	...
	function init(){
		...
		// var clientDigest = location.search.split("?")[1].split("&")[1].split("=")[1];
		// alert(username);
		$.ajax({
			type:'get',
...
//					var createTime = new Date(parseInt(listPageObject[i].createTime) * 1000).toLocaleString());
					...
					var t = {id:data[i].id,申请人:data[i].enterpriseName,创建时间:time,审核状态:status,审核:'审核操作'}
				...
				jQuery(function($) {
					...
					jQuery(grid_selector).jqGrid({
						...
						datatype: "local",
						height: "100%",
						colNames:['ID','申请人','创建时间', '审核状态','审核'],
						colModel:[
							{name:'id',index:'id', width:15, sorttype:"int", editable: true},
							{name:'申请人',index:'申请人', width:25, sorttype:"int", editable: true},
...
//							{name:'下属职位', index:'下属职位',editable: true, edittype: 'select',editrules:{required:true}, editoptions: { value: { 1: 'One', 2: 'Two'}} }
							...
							{name:'审核状态',index:'审核状态', width:25, sorttype:"int", editable: true},
							{name:'审核',index:'审核', width:25, sorttype:"int", editable: true,},
						],
...
//						},
	...
	//					caption: "jqGrid with inline editing",//表格名称
					...
					*/
					...
					//申请人审核
					$("#grid-table tbody").find("tr").find("td:nth-child(5)").click(function(){
