...
ctxtObj.strokeRect(j*cellW+0.5,(rC-i)*cellH+curOff+0.5,cellW-1,cellH-1);
	...
	function animate(time){
		if(isStart){
			...
			btn_start.innerHTML="跨越: "+goodJump +" 块<br />剩余时间: "+ hasTime + " 秒";
			if(errorTime>=bigError || hasTime<=0){
				isStart=false;
				btn_start.innerHTML+="<br />点击底部黑块重新开始<br />";
//				btn_start.innerHTML+="<br />点击底部黑块重新开始<br />"<div style='color:white;background-color:"+(goodJump>=winCount?"red":(goodJump>=normalCount?"green":(goodJump<=lostCount?"gray":"green")))+";font-size:40px;'>"+(goodJump>=winCount?"高手":(goodJump>=normalCount?"还行":(goodJump<=lostCount?"太糟了":"还行")))+"，60秒内跨越 "+goodJump +" 块"+"</div>";
				var id = $("#id").text();
				$.ajax({	
					type:"get",  
					url:"http://wx.yulin520.com/a2a/wechatChristmas/save2",  
					data: {"openId":id,"blocks":goodJump},
					dataType:"json",
