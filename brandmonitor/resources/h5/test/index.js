...
function loadata_yl(pageIndex){
			...
			$.ajax({ 
				type:"GET",  
				url:"http://www.yulin520.com:8082/a2a/news/yzyl/list",  
				data: {"page":pageIndex},
				async:false, 
				dataType:"json", 
...
//					    $('#loading').attr('src', data[i].img);
//					  }
//					};
//					$.preloadImages('data[i]+url');
					
					for(i=0;i<length;i++){
						
						if(i!=length){
							$('div.ylsection').append(
...
//					$("append_img img").lazyload({ 
//						placeholder : "../../../resources/h5/test/images/loading.jpg", 
//						effect : "fadeIn" ;
//					}); 
//					var t_img; // 定时器
...
//					$("#loading").hide();
		...
		function loadata_act(pageIndex,pageSize){
			$.ajax({ 
				type:"GET",  
				url:"http://www.yulin520.com:8082/a2a/events/list",  
		...
		function loadata_help(pageIndex,pageSize){
			$.ajax({ 
				type:"GET",  
				url:"http://www.yulin520.com:8082/a2a/hp/list",  
				data: {"page":pageIndex,"pageSize":pageSize,},
				dataType:"json",
				beforeSend: function(request) {
                        request.setRequestHeader("communityId", "1");
