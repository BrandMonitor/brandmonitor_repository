...
carousels.each(function() {
  ...
  if ($obj.data('shift') === 1) {
    var items = $obj.find('.item1 > [class*="col-"]'),
        visibleCnt = $obj.find('.item1:first [class*="col-"]').length,
        wrapper = "";
    
    // build styles
    var rule_base = '.carousel.' + id + ' .carousel-inner > .item1',
        styles = $('<style></style>'),
        rules = [];
