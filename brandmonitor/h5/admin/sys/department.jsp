		...
		<div class="edit" id="edit">
			...
			<div class="content">
				<div class="widget-header">
				  <span class="ui-jqdialog-title" style="float: left;">新增部门</span>	<a href="javascript:void(0)" class="ui-jqdialog-titlebar-close ui-corner-all" style="right: 0.3em;">
					...
					<table id="TblGrid_grid-table" class="" cellspacing="0" cellpadding="0" border="0">
					  <tbody>
					    ...
					    <tr rowpos="5" class="FormData" id="tr_department">
					      ...
					      <td class="DataTD">&nbsp;<input type="text" id="department" name="department" role="textbox" class="FormElement ui-widget-content ui-corner-all">
...
//					alert(time);
//					var createTime = listPageObject[i].createTime;
//					var time = createTime.toLocaleDateString();
					if(listPageObject[i].status==1){
						var status = '存在';
					}else{var status = '不存在';}
					var t = {id:listPageObject[i].id,部门:listPageObject[i].departmentName,创建时间:time,状态:status,备注:listPageObject[i].remark}
				...
				jQuery(function($) {
					...
					jQuery(grid_selector).jqGrid({
						...
						datatype: "local",
						height: "100%",
						colNames:['ID','部门','创建时间', '状态','备注'],
						colModel:[
							{name:'id',index:'id', width:15, sorttype:"int", editable: true},
							{name:'部门',index:'部门', width:25, sorttype:"int", editable: true},
//							{name:'下属职位','下属职位',editable:true,edittype:'select',editrules:{required:true},editoptions:{size:71},align:'center',sortable:false,search:true},
//							{name:'下属职位', index:'下属职位',editable: true, edittype: 'select',editrules:{required:true}, editoptions: { value: { 1: 'One', 2: 'Two'}} }
...
//							{name:'备注',index:'备注', width:25, sorttype:"int", editable: true},
							{name:'备注',index:'备注', width:25, fixed:true, sortable:false, resize:false,
							formatter:'actions', 
							formatoptions:{ 
								keys:true,
